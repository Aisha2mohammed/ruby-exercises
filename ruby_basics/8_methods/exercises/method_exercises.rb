...
def ascii_translator(number)
 number.chr  if    number.is_a?(Integer) && number.between?(0,127)
end
...
def ascii_code(letter)
    if letter.is_a?(String) && letter.length == 1
      letter.ord  # Get the ASCII code of the single character
    else
      'Input Error'
    end
  end
